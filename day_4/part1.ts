...
function includes(interval1: Interval, interval2: Interval): boolean {
...
const count = pairs.reduce((acc, x: Pair) => acc + Number(includes(x[0], x[1]) || includes(x[1], x[0])), 0);
console.log("Number of contained intervals: ", count);
